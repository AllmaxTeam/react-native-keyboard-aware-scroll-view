...
const KeyboardAwareMixin = {
  ...
  blockScrollToDefaultResetCoords: function () {
    this.isBlockingScrollToDefaultResetCoords = true
  },

  // Keyboard actions
  updateKeyboardSpace: function (frames: Object) {
    const keyboardSpace: number = frames.endCoordinates.height - this.props.bottomOffset
        ...
        (isAncestor) => {
          if (isAncestor) {
            UIManager.measureInWindow(currentlyFocusedField, (x, y, width, height) => {
              if (y + height - this.props.bottomOffset > frames.endCoordinates.screenY - this.props.extraHeight) {
    ...
    if (!this.resetCoords) {
      if (!this.isBlockingScrollToDefaultResetCoords) {
        if (!this.defaultResetScrollToCoords) {
          this.defaultResetScrollToCoords = this.position
        }
      } else {
        this.isBlockingScrollToDefaultResetCoords = false
      }
    }
  },
